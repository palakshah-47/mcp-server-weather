	...
	return [
		{
			...
			displayName: 'GPT-4o',
			publisher: 'OpenAI',
			summary: "OpenAI's most advanced multimodal model",
		...
		},
		{
			...
			publisher: 'OpenAI',
			summary: 'Smaller, efficient version of GPT-4o',
			context_window: 128000,
		},
		{
			...
			publisher: 'Microsoft',
			summary: 'A mixture of experts model from Microsoft',
		...
		},
		{
			...
			publisher: 'Microsoft',
			summary: 'Small model with large context window',
			context_window: 131072,
		},
		{
			...
			publisher: 'Meta',
			summary: 'Advanced reasoning and instruction following',
			context_window: 128000,
		},
		{
			...
			publisher: 'Meta',
			summary: 'Balanced performance and efficiency',
			context_window: 8192,
		},
		{
			id: 'Mistral-large',
			...
			summary: "Mistral's flagship model for complex reasoning",
		...
		},
		{
			...
			publisher: 'Mistral AI',
			summary: 'Efficient model for low-latency use cases',
			context_window: 32768,
		},
	];
}
	...
	for (const model of models) {
		try {
			...
			if (axios.isAxiosError(error)) {
				responses[model] = `Error: ${error.response?.status} ${
...
server.tool(
	'compare_models',
	'Compare responses from different language models for a given prompt',
	{
				...
				content: [
					{
						...
						text: JSON.stringify(
							{
								error: 'No valid model IDs provided for comparison.',
			...
			content: [
				{
					type: 'text',
					text: JSON.stringify(
						{ results: responses, summmary: { models_compared: validModels, prompt } },
						null,
						2
					),
				},
...
server.prompt(
	...
	'Create a comparison prompt with selected models',
	{
		...
		models: z.string().optional().describe('Comma-separated list of model IDs to compare'),
	},
	({ prompt, models }: CompareModelsPromptParams) => {
		const message = models
			...
			: `Please compare how different AI models respond to this prompt:\n\nPrompt: ${prompt}\n\nYou can choose popular models for comparison.`;
						...
						text: JSON.stringify(
							{
								...
								count: models.length,
								publishers: [
									...new Set(models.map((model) => model.publisher)),
								].sort(),
							},
							null,
							2
